...
class cascadeFeatureFusion(nn.Module):
    ...
    def forward(self, x_low, x_high):
        ...
        low_depth = self.low_depth_conv(x_low_upsampled)
        ...
        return high_fused_fm, low_cls, low_depth
...
def get_multiply_scale_inputs(input, scales=[4, 8, 16]):
    inputs = []
    for s_scale in scales:
        inputs.append(
            F.interpolate(input, size=get_interp_size(input, s_factor=s_scale), mode="bilinear", align_corners=True))
    return tuple(inputs)
...
class BatchNorm1d:
    ...
    def backward(self, d_loss):
        ...
        u = self.gamma / shape_t[1] / self.sqrt
        u = u.T
        y = r * u[:, np.newaxis]
...
def convert_param_dict(model_path):
