...
class Encoder(nn.Module):
    def __init__(self, num_classes):
        ...
        for x in range(0, 3):
            self.layers.append(ResModule(32, 0.03, 1))

        self.layers.append(DownsamplerBlock(32, 64))

        for x in range(0, 2):
            self.layers.append(ResModule(64, 0.03, 1))

        self.layers.append(DownsamplerBlock(64, 128))

        for x in range(0, 1):
            self.layers.append(ResModule(128, 0.3, 1))
            self.layers.append(ResModule(128, 0.3, 2))
            self.layers.append(ResModule(128, 0.3, 5))
            self.layers.append(ResModule(128, 0.3, 9))

        for x in range(0, 1):
            self.layers.append(ResModule(128, 0.3, 2))
            self.layers.append(ResModule(128, 0.3, 5))
            self.layers.append(ResModule(128, 0.3, 9))
            self.layers.append(ResModule(128, 0.3, 17))
...
class Decoder(nn.Module):
    def __init__(self, num_classes):
        ...
        self.fpa = FPAModule(in_ch=128, out_ch=num_classes)
...
class spnet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.encoder = Encoder(num_classes)
        self.decoder_seg = Decoder(num_classes)
        self.decoder_depth = Decoder(1)

    def forward(self, x):
        x = self.encoder(x)
        seg_logits = self.decoder_seg(x)
        depth = self.decoder_depth(x)
...
if __name__ == '__main__':
    ...
    batch = 4
    x = torch.rand(batch, 3, 512, 1024).cuda()
    net = spnet(20).cuda()
    # params_size(net)
    with torch.no_grad():
        import time
