...
class icnet(nn.Module):
    ...
    def __init__(
        ...
        self.res_block3_conv = residualBlockPSP(
            self.block_config[1], 128, 64, 256, 2, 1, include_range="conv",is_batchnorm=is_batchnorm)

        self.res_block3_identity = residualBlockPSP(
            self.block_config[1], 128, 64, 256, 2, 1, include_range="identity",is_batchnorm=is_batchnorm)
        ...
        self.depth_estimate = nn.Conv2d(128, 1, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        x_sub24, sub4_cls, sub4_depth = self.cff_sub24(x_sub4, x_sub2)
        x_sub12, sub24_cls, sub24_depth = self.cff_sub12(x_sub24, x_sub1)
        ...
        sub124_depth = self.depth_estimate(x_sub12)

        sub124_depth = F.interpolate(sub124_depth, size=get_interp_size(sub124_depth, z_factor=4),
                                     mode="bilinear", align_corners=True)
        sub124_cls = F.interpolate(sub124_cls, size=get_interp_size(sub124_cls, z_factor=4),
                                   mode="bilinear", align_corners=True)

        if self.multi_results:
            sub4_depth = F.interpolate(sub4_depth, size=get_interp_size(sub4_depth, z_factor=16),
                                       mode="bilinear", align_corners=True)
            sub4_cls = F.interpolate(sub4_cls, size=get_interp_size(sub4_cls, z_factor=16),
                                       mode="bilinear", align_corners=True)

            sub24_depth = F.interpolate(sub24_depth, size=get_interp_size(sub24_depth, z_factor=8),
                                       mode="bilinear", align_corners=True)
            sub24_cls = F.interpolate(sub24_cls, size=get_interp_size(sub24_cls, z_factor=8),
                                       mode="bilinear", align_corners=True)

            return [(sub4_depth, sub4_cls), (sub24_depth, sub24_cls), (sub124_depth, sub124_cls)]

        else:
            return sub124_depth, sub124_cls


if __name__ == "__main__":
    net = icnet(is_batchnorm=True).cuda()
    from torchsummary import summary
    summary(net, (3, 513, 1025))
